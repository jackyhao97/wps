...
function ShowImages($data, $judul) {
  $component = '<a href="'.BASE_URL.DS.'admin/img/services/'.$data.'" data-lightbox="'.$judul.'" data-title="'.$judul.'">';
  $component .= '<img src="'.BASE_URL.DS.'admin/img/services/'.$data.'" class="w-50" />';
  $component .= "</a>";
...
$table = <<<EOT
  (
    SELECT `id`, `judul`, `keterangan`, `path` FROM `tb_services`
  ) temp 
  EOT;

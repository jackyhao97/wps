  ...
  if (isset($_POST['hid_id'])) {
    ...
    if ($block == 0) {
      if ($file != '') {
        ...
        $upload = move_uploaded_file($file['tmp_name'], "img/career/".$new_filename);
        $update_sql = "UPDATE tb_career SET modified_on = '$modified_on', modified_by='$user', judul='$judul', keterangan='$keterangan', path='$new_filename' WHERE id='$id'";
      }
      else {
        $update_sql = "UPDATE tb_career SET modified_on = '$modified_on', modified_by='$user', judul='$judul', keterangan='$keterangan' WHERE id='$id'";
      }
